...
class FastCGIRecordDecoder: FastCGIDecoder {
    ...
    private func parseData() throws {
        if contentLength > 0 {
            let end = min(pointer + Int(contentLength), buffer.count)
            data = buffer.subdata(in: pointer..<end)
    ...
    static func decode(from data: Data) -> [FastCGIRecord] {
        ...
        while remainingData != nil {
            ...
            do {
                ...
                // Cannot decode FastCGI response data anymore!
                remainingData = nil
            }
            ...
            switch type {
            case .beginRequest:
                content = .beginRecordContent(decoder.role, decoder.keepalive ? 1 : 0)
            case .endRequest:
            ...
            records.append(FastCGIRecord(version: decoder.version,
                                         type: FastCGIRecord.RecordType(rawValue: decoder.type)!,
